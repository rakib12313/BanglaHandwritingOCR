  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      try {
        ...
        // Fallback or ignore if process is undefined
      }

      // Check if key exists
      if (!envKey) {
        throw new Error("API_KEY is missing. Please check your .env file (local) or Vercel Environment Variables.");
      }
      ...
      if (cleanKey.length === 0) {
        throw new Error("API_KEY is empty. Please check your .env file.");
      }
      ...
      if (msg.includes("API key not valid") || msg.includes("API_KEY_INVALID")) {
        msg = "Invalid API Key. Please verify the 'API_KEY' in your .env file or Vercel Settings.";
      } else if (msg.includes("process") || msg.includes("API_KEY")) {
        msg += " (Check Environment Variables)";
      }

      setError("Failed to process document. " + msg);
    } finally {
